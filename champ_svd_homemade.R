# This is an altered version of the champ.SVD function
champ.SVDpro <- function (beta = myNorm, rgSet = NULL, pd = myLoad$pd, RGEffect = FALSE, 
          PDFplot = TRUE, Rplot = TRUE, topPCA = 20, figurename = "SVDsummary.pdf", resultsDir = "./CHAMP_SVDimages/")
{
  message("[===========================]")
  ...
  EstDimRMTv2 <- function(data.m) {
    M <- data.m
    for (c in 1:ncol(M)) M[, c] <- (data.m[, c] - mean(data.m[, 
                                                              c]))/sqrt(var(data.m[, c]))
  ...
  drawheatmap <- function(svdPV.m, svd.o, rmt.o) {
    x = svd.o
    y = rmt.o
    scp <- list(u = x$u[1:nrow(x$u), 1:y$dim], v = x$v[1:y$dim, 
                                                         1:y$dim], d = x$d[1:y$dim])
    percent_component <- round(scp$d^2/sum(scp$d^2) * 100, 1)

    my_xlabels <- paste0("PC-", 1:nrow(svdPV.m), " (", percent_component[1:topPCA], "%)")
    ...
    axis(1, at = 1:nrow(svdPV.m), labels = my_xlabels, las = 2)
    suppressWarnings(axis(2, at = 1:ncol(svdPV.m), labels = colnames(svdPV.m), 
                          las = 2))
    legend(x = topPCA+1, y = ncol(pd)+2 , legend = c(expression("p < 1x" ~ 
                                                             10^{
                                                               -10
                                                             }), expression("p < 1x" ~ 10^{
                                                               -5
                                                             }), "p < 0.01", "p < 0.05", "p > 0.05"), fill = c("darkred", 
                                                                                                               "red", "orange", "pink", "white"), par("usr")[2], 
           par("usr")[4], xpd = NA)
  }
  ...
  svd.scree <- function(svd.obj, maintitle = "Scree Plot", 
                        axis.title.x = "Component Index (Singular Vectors)", 
                        axis.title.y = "Percent Variance Explained") {
    if (is.list(svd.obj) & all(names(svd.obj) %in% c("u", 
                                                     "d", "v"))) {
    ...
    g = ggplot2::ggplot(pve, aes(names, pve)) + geom_point() + 
      theme_bw()
    gout = g + ylim(0, NA) + ggtitle(maintitle) + xlab(axis.title.x) + 
      ylab(axis.title.y)
    return(gout)
  }
  ...
  message("[SVD analysis will be proceed with ", dim(beta)[1], 
  ...
  if (class(pd) == "matrix") 
    pd <- as.data.frame(pd)
  PhenoTypes.lv_tmp <- pd[, !colnames(pd) %in% c("Sample_Name", 
                                                 "Project", "filenames", "Basename") & apply(pd, 2, function(x) length(unique(x))) != 
                            1]
  ...
  if (ncol(PhenoTypes.lv) >= 2) {
    ...
    sapply(colnames(PhenoTypes.lv_tmp), function(x) message("<", 
                                                            x, ">(", class(PhenoTypes.lv[[x]]), "):", paste(unique(PhenoTypes.lv_tmp[, 
                                                                                                                                     x]), collapse = ", ")))
  ...
  if (!is.null(rgSet) & RGEffect) {
    ...
    dataC2.m <- as.data.frame(log2(apply(ControlProbes, 1, 
                                         function(x) if (x[3] == "Grn") getGreen(rgSet)[x[2], 
                                         ] else getRed(rgSet)[x[2], ])))
  ...
  else stop("Dimension of your pd file (and rgSet information) is not equal to your beta matrix.")
  ...
  for (c in 1:topPCA) for (f in 1:ncol(PhenoTypes.lv)) if (class(PhenoTypes.lv[, 
                                                                               f]) != "numeric") 
    svdPV.m[c, f] <- kruskal.test(svd.o$v[, c] ~ as.factor(PhenoTypes.lv[[f]]))$p.value
  else svdPV.m[c, f] <- summary(lm(svd.o$v[, c] ~ PhenoTypes.lv[[f]]))$coeff[2, 
                                                                             4]
  ...
  if (PDFplot) {
    pdf(paste(resultsDir, figurename, sep = ""), width = 12, 
        height = 8)
    par(mar = c(10, 10, 10, 10))
    drawheatmap(svdPV.m, svd.o, rmt.o)
    splot(svd.o, rmt.o)
    dev.off()
  }
